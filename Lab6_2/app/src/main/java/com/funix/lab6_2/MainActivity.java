...
public class MainActivity extends AppCompatActivity implements View.OnClickListener, SeekBar.OnSeekBarChangeListener {
    ...
    private void loadingListSongOffline() {
        ...
        for (Field field : fields) {
            try {
                String name = getResources().getResourceEntryName(field.getInt(null));
                String path = "android.resource://" + getPackageName() + "/" + field.getInt(null);
                int rawResourceId = field.getInt(null);
                listSong.add(new SongEntity(name, path, "N/A",rawResourceId));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
 ...
 public void onClick(View v) {
    ...
    private void play() {
        ...
        player.reset();
        try {
            AssetFileDescriptor afd = getResources().openRawResourceFd(songEntity.getRawResourceId());
